...
FROM (
        SELECT a.age_name,
            ...
            TRIM(a.cli) cli_id,
            a.nomrest names,
            nvl(aut.maut * taux.tind, a.balance) maut,
            a.balance,
            a.balance_fcy,
        ...
        FROM (
            ...
            AND intsus.dco = ? and a.balance<0
            ...
            left join (
                ...
                where dco = ?
                    and cha = '299120'
                    and sde > 0
            ) prov_held on trim(prov_held.cli) = trim(a.cli) --and a.balance<0
            left join (
                select cli,
                    sum(round(ctvmon)) collat
                from prod.miscolatctv
                group by cli
            ) col on trim(col.cli) = trim(a.cli)-- and a.balance<0
            LEFT JOIN (
                SELECT a.age,
                    a.lien,
                    a.datr,
        ...
        group by a.age_name,
            ...
            TRIM(a.cli),
            a.nomrest,
            nvl(aut.maut * taux.tind, a.balance),
            a.balance,
            case
                when b.debut is not null then b.debut
                when b.debut is null
