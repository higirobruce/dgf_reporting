...
main.dev,
main.sdecv,
nvl(main.cap_imp,0) cap_imp,
nvl(main.int_imp,0) int_imp,
nvl(main.comm_imp,0) comm_imp,
nvl(main.cap_imp_npl,0) cap_imp_npl,
nvl(main.com_imp_npl,0) com_imp_npl,
nvl(main.int_imp_npl,0) int_imp_npl,
nvl(main.prov_int,0) prov_int,
main.newcla,
main.ndaysarr,
...
if ($stmt->execute(array(
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($prev_cli == (string) trim($row['CLI']) && $prev_cap == $row['CAP_IMP']) {
            ...
            $t_cap_imp = abs((float) $row['CAP_IMP']);
        }
        ...
        if ($prev_cli == (string) trim($row['CLI']) && $prev_prov_int == $row['PROV_INT']) {
            ...
            $t_prov_int = abs((float) $row['PROV_INT']);
        }
        ...
        if ($prev_cli == (string) trim($row['CLI']) && $prev_int == $row['INT_IMP']) {
            ...
            $t_int_imp = abs((float) $row['INT_IMP']);
        }
        ...
        if ($prev_cli == (string) trim($row['CLI']) && $prev_int_npl == $row['INT_IMP_NPL']) {
            ...
            $t_int_imp_npl = abs((float) $row['INT_IMP_NPL']);
        }
        ...
        if ($prev_cli == (string) trim($row['CLI']) && $prev_com == $row['COMM_IMP']) {
            ...
            $t_comm_imp = abs((float) $row['COMM_IMP']);
        }
        ...
        if ($prev_cli == (string) trim($row['CLI']) && $prev_com_npl == $row['COM_IMP_NPL']) {
            ...
            $t_comm_imp_npl = abs((float) $row['COM_IMP_NPL']);
        }
        ...
        if ($prev_cli == (string) trim($row['CLI']) && $prev_capn == $row['CAP_IMP_NPL']) {
            ...
            $t_cap_imp_npl = abs((float) $row['CAP_IMP_NPL']);
        }
        ...
        if ($prev_cli == (string) trim($row['CLI']) && $prev_amount == $row['LOAN_AMOUNT']) {
            ...
            $t_loan_amount = abs((float) $row['LOAN_AMOUNT']);
        }


        // $t_cap_imp != 0 && $t_cap_imp_npl != 0 && $t_comm_imp != 0 && $t_comm_imp_npl != 0 && $t_int_imp != 0 && $t_int_imp_npl != 0
        //     && $t_prov_int != 0 && $t_sdecv != 0

        if ($t_cap_imp != 0 && $t_sdecv != 0) {
            ...
            $prov_int[] = $t_prov_int;
            $int_imp[] = $t_int_imp;
            $int_imp_npl[] = $t_int_imp_npl;
            $comm_imp[] = $t_comm_imp;
            $comm_imp_npl[] = $t_comm_imp_npl;
            $cap_imp_npl[] = $t_cap_imp_npl;
