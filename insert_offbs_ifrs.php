...
if (isset($_GET['hidden_month_of'])) {
    $month = $_GET['hidden_month_of'];
    $year = $_GET['hidden_year_of'];
} else {
    $month = 6;
    $year = 2018;
}
...
$query = "SELECT distinct a.age,
    ...
    a.beneficiary,
    a.currency,
    nvl(b.mon,a.balance_lcy) amount,
    ...
    chap.lib cha_lib,
    a.age_code,
    -- nvl(nvl(classif.newcla,classif_live.newcla), 6) newcla,
    nvl( nvl(classif_live.newcla,classif.newcla), 6) newcla,
    nvl( nvl(classif_live.ndaysarr,classif.ndaysarr), 0) ndaysarr,
    a.prov_held,
...
from (
        select trim(d.lib) age,
            ...
            nvl(nvl(cl.newcla,b.newcla),6) class_risk,
        ...
        from prod.bksld a
            ...
            left join (
                ...
                where dco = ?
                    and cha = '519000'
                    and sde > 0
            ) prov_held on prov_held.cli = a.cli --and a.sde<0
            left join (
                select cli,
    ...
    left join prod.misnewclassfinal classif_live on trim(classif_live.cli) = trim(a.cli) and prod.month(classif_live.finmois) = ? and prod.year(classif_live.finmois)=?
...
group by a.age,
    ...
    a.beneficiary,
    a.currency,
    nvl(b.mon,a.balance_lcy),
    nvl(b.dco, sysdate -365),
    nvl(b.dech, sysdate),
    chap.lib,
    a.age_code,
    nvl( nvl(classif_live.newcla,classif.newcla), 6),
    nvl( nvl(classif_live.ndaysarr,classif.ndaysarr), 0),
    a.prov_held,
    a.collateral_val
order by a.cli asc";
...
if ($stmt->execute(array(
    $dco, $month, $year, $dco, $dco, $month, $year, $month, $year
))) {
    $i = 0;
    $j = 0;
    $prev_balance = 0;
