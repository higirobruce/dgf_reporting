...
//dco
if (isset($_GET['hidden_month_tl'])) {
    $month = $_GET['hidden_month_tl'];
    $year = $_GET['hidden_year_tl'];
} else {
    $month = 6;
    $year = 2020;
}

$q_delete = "DELETE FROM clearinguser.ifrs9_tloans";
...
nvl(nvl(cl.newcla,cl_live.newcla),6) newcla,
cl.ndaysarr,
...
nvl(int_due.sdecv,0) int_due_lcy,
case when a.dev<>646 then nvl(int_due.sde,0) else 0 end int_due_fcy,
nvl(charges.sdecv,0) charges
...
left join (SELECT cli,sum(sdecv) sdecv,sum(sde) sde from prod.bksld where dco=?
and cha in ('290610','290612','290614','290620','290630','290640','290664','290670',
...
left join (SELECT cli,sum(abs(sdecv)) sdecv,sum(abs(sde)) sde from prod.bksld where dco=?
and cha in ('219010','219200','229200','239200','249200','290810','290840','290864','290874',
'290890','290892','290910','290912','290964','290974','290980','290983','290990',
'290992','290812','290920','290940','290820','290880','290991','290891',
'290810','290910','290964','291380','291390','291480','291490','291580',
'291590','291691','291692','291761','291762','291862','291961','291962') and sde<0
group by cli) int_due on int_due.cli = a.cli


left join (SELECT cli,sum(abs(sdecv)) sdecv,sum(abs(sde)) sde from prod.bksld where dco=?
and cha in ('472201','472202')  and sde>0
group by cli) charges on charges.cli = a.cli
...
left join PROD.MISNEWCLASSFINAL  cl_live on trim(a.cli) = trim(cl_live.cli) and prod.month(cl_live.finmois) = prod.month(a.dco) and prod.year(cl_live.finmois)=prod.year(a.dco)
...
(
    sdecv >= 0 and (
            ...
            com_imp_npl is not null or int_imp_npl is not null or prov_int is not null or agio_cdt_march is not null or
            int_due_lcy is not null
...
if ($stmt->execute(array(
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $int_due_lcy[] = abs($row['INT_DUE_LCY']);
        $int_due_fcy[] = abs($row['INT_DUE_FCY']);
        $charges[] = $row['CHARGES'];
        ...
        if (
            ...
            array_pop($int_due_lcy);
            array_pop($int_due_fcy);
            array_pop($charges);
        }
...
for ($n = 0; $n < sizeof($nomrest); $n++) {
    ...
    $interest_due_lcy = $int_due_lcy[$n];
    ...
    $interest_due_fcy = $int_due_fcy[$n];
    ...
    $amount_due =  ($int_imp[$n]+$comm_imp[$n]+abs($int_imp_npl[$n])+abs($comm_imp_npl[$n])+$prov_int[$n]+$agio_cdt_march[$n])+$capital_due_lcy;
...
$q_insert_amount = "INSERT INTO clearinguser.ifrs9_cli_loanamount_tl
    SELECT cli, sum(LOAN_includ_interest),dco from clearinguser.ifrs9_tloans  where doc_type in ('TL') --and loan_amount<>0 
    group by cli,dco";
...
set a.interest_susp=(
    ...
    where trim(a.cli)=trim(b.cli) and a.dco=b.dco and b.loan_amount<>0
) where exists
(
select a.cli from clearinguser.ifrs9_tloans a
join clearinguser.ifrs9_cli_loanamount_tl b on trim(a.cli)=trim(b.cli) and a.dco=b.dco and b.loan_amount<>0
)";
...
// update Interest due
$q_update = "UPDATE clearinguser.ifrs9_tloans a
set a.interest_due_lcy=(
    select round(a.interest_due_lcy * (a.LOAN_includ_interest/b.loan_amount),0) from clearinguser.ifrs9_cli_loanamount_tl b
    where trim(a.cli)=trim(b.cli) and a.dco=b.dco and b.loan_amount<>0 and a.doc_type in ('TL')
) where exists
(
select a.cli from clearinguser.ifrs9_tloans a
join clearinguser.ifrs9_cli_loanamount_tl b on trim(a.cli)=trim(b.cli) and a.dco=b.dco and b.loan_amount<>0 and a.doc_type in ('TL')
) and a.doc_type='TL' and a.sdecv<>0";
...
//update Capital Due
// $q_update = "UPDATE clearinguser.ifrs9_tloans a
// set a.capital_due_lcy=(
//     select round(a.capital_due_lcy * (a.LOAN_includ_interest/b.loan_amount),0) from clearinguser.ifrs9_cli_loanamount_tl b
//     where trim(a.cli)=trim(b.cli) and a.dco=b.dco and b.loan_amount<>0 and a.doc_type in ('TL')
// ) where exists
// (
// select a.cli from clearinguser.ifrs9_tloans a
...
$q_update = "UPDATE clearinguser.ifrs9_tloans a
set a.other_charges=(
    select round(a.other_charges * (a.LOAN_includ_interest/b.loan_amount),0) from clearinguser.ifrs9_cli_loanamount_tl b
    where trim(a.cli)=trim(b.cli) and a.dco=b.dco and b.loan_amount<>0 and a.doc_type in ('TL')
) where exists
(
select a.cli from clearinguser.ifrs9_tloans a
